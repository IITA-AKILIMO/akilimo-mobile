...
- Fix(locale): persist language selection using full BCP-47 tags (MUN-16)

Four compounding bugs caused language changes to revert to English on restart:

1. Language code stored as short form ("sw") instead of full BCP-47 tag
   ("sw-TZ"), preventing Android resource resolution from matching
   values-sw-rTZ/ on API 24+ strict locale matching.
   - WelcomeFragment: it.language → it.toLanguageTag()
   - UserSettingsActivity: hardcoded options → derived from Locales.supportedLocales
   - SessionManager: default fallback updated to Locales.english.toLanguageTag()

2. AppLocale.desiredLocale never set from user selection, leaving the
   Reword string-replacement library on the default (English) locale.
   - Both WelcomeFragment and UserSettingsActivity now call
     AppLocale.desiredLocale = selectedLocale after persisting the choice.
   - AkilimoApp.initLocale() falls back to SessionManager.languageCode
     when SharedPrefsAppLocaleRepository has no stored value, keeping the
     two sources in sync on every process start.

3. WelcomeFragment used a manual activity restart (finish + startActivity)
   which skipped AkilimoApp.onCreate() and therefore skipped initLocale().
   Replaced with ProcessPhoenix.triggerRebirth() to match UserSettingsActivity.

4. WelcomeFragment did not write the language choice to UserPreferences,
   causing prefillFromEntity() to display a stale value on re-entry.
   Now saves to UserPreferences.languageCode alongside AkilimoUser.

Co-Authored-By: Claude Sonnet 4.6 <noreply@anthropic.com>

- Fix(workmanager): implement Configuration.Provider for on-demand initialization

WorkManager.getInstance() was called in AkilimoApp.onCreate() via
WorkerScheduler before the androidx.startup InitializationProvider
had completed, causing an IllegalStateException on app start.

Implement Configuration.Provider in AkilimoApp so WorkManager
initializes lazily on first getInstance() call rather than relying
on the startup-library auto-initializer timing.

- AkilimoApp now implements Configuration.Provider and returns a
  Configuration with DEBUG-level logging in debug builds.
- AndroidManifest removes the WorkManagerInitializer meta-data entry
  from InitializationProvider (tools:node="remove") to prevent the
  auto-initializer from running in parallel and conflicting with the
  on-demand path.

Co-Authored-By: Claude Sonnet 4.6 <noreply@anthropic.com>

- Fix(ui): apply dark mode preference on app start

MODE_NIGHT_NO was hardcoded in BaseActivity.onCreate(), silently
discarding the UserPreferences.darkMode flag set in UserSettingsActivity.

- SessionManager: add darkMode: Boolean backed by SharedPreferences
  so the preference is readable synchronously before any Activity inflates
- AkilimoApp.onCreate(): call initDarkMode() which reads SessionManager
  .darkMode and calls AppCompatDelegate.setDefaultNightMode() once per
  process start, before any Activity is created
- UserSettingsActivity: sync sessionManager.darkMode = preferences.darkMode
  on save so the cached value stays in sync with Room
- BaseActivity: remove hardcoded setDefaultNightMode(MODE_NIGHT_NO);
  remove now-unused AppCompatDelegate and Locales imports

Co-Authored-By: Claude Sonnet 4.6 <noreply@anthropic.com>

- Fix(ui): resolve hardcoded colors breaking dark mode in layouts and drawables

StepperLayout bottom navigation bar:
- Add app:ms_bottomNavigationBackground="?attr/colorSurface" to StepperLayout
  in activity_home_stepper.xml; the library defaults to white (#FFFFFF)
  which was the reported white bottom navbar in dark mode

fragment_welcome.xml:
- MaterialCardView language card: @color/white → ?attr/colorSurface so the
  card background tracks the theme surface color instead of forcing white

fragment_terms.xml:
- WebView background: @android:color/background_dark (always black) →
  ?android:attr/colorBackground so it follows the current theme

Drawables (ic_info_outline, ic_language, ic_location_plus):
- Remove android:tint="#000000" from all three vector drawables; tint is
  already applied contextually in each layout via app:tint="?attr/..."
  so the hardcoded black was redundant and would make icons invisible
  anywhere the layout-level tint was absent

Co-Authored-By: Claude Sonnet 4.6 <noreply@anthropic.com>

- Fix(ui): consistent image-tap selection UX across yield and performance screens

- CassavaYieldAdapter: split bind() into bind(item) + applySelectionState(); add
  animated card background, 2dp primary stroke border, selectionOverlay and
  selectionIndicator visibility toggle; fix DiffCallback.areItemsTheSame to use
  entity id instead of imageRes+label
- MaizePerformanceAdapter: align to same pattern — add applySelectionState() with
  animateCardBackground, strokeWidth, selectionOverlay/selectionIndicator; fix
  DiffCallback.areItemsTheSame to compare enum value directly; remove unused
  position param from bind()
- SelectedCassavaMarketRepo: include yieldId in select() copy so yield selection
  persists when the user re-opens the screen with an existing record
- item_card_recommendation_image.xml: add strokeColor="?attr/colorPrimary" so
  adapters can dynamically control the selection border width

Co-Authored-By: Claude Sonnet 4.6 <noreply@anthropic.com>

- Fix(ui): dropdown options empty after ProcessPhoenix restart

All MaterialAutoCompleteTextView dropdowns (language, country, area unit,
gender, etc.) appeared empty when opened after a ProcessPhoenix rebirth.

Root cause: ArrayAdapter.getFilter() returns ArrayFilter which calls
performFiltering(currentText) when the dropdown popup opens. After
prefillFromEntity()/populateForm() sets a display value via
setText("English", false), that text is used as the filter constraint.
The ArrayFilter matches items by item.toString(), which for data-class
option objects doesn't align with getDisplayText(), so zero items pass
the filter and the popup appears empty.

Fix: override getFilter() in BaseSpinnerAdapter to return a no-op filter
that always returns the full options list. These are exposed-dropdown-menu
components (selection pickers, not search inputs) — filtering is never
desired. Every subclass (ValueOptionAdapter, BaseValueOptionAdapter) inherits
...
- Docs: add PRD, roadmap, user guide and expand architecture/development docs

Adds a full documentation set derived from codebase analysis:

- docs/PRD.md: product purpose, target users, current features, known
  gaps, functional and non-functional requirements (including MUN-16)
- docs/ROADMAP.md: phased improvement plan across short (0-4 weeks),
  medium (1-3 months), and long-term (3-6 months) horizons with effort,
  impact, and dependency annotations
- docs/USER_GUIDE.md: onboarding walkthrough, main user flows, crop
  workflow reference table, and known limitations for end users

Updated existing docs:
- docs/ARCHITECTURE.md: full rewrite with layer diagram, package table,
  startup sequence, locale/i18n system flow, navigation map, state
  management summary, and technical debt register
- docs/DEVELOPMENT.md: added package conventions, stepper/activity
  patterns, locale change rules, coroutine guidelines, and active bug table
- README.md: fixed incorrect "French" language entry (app supports
  Kinyarwanda, not French); added new docs to documentation index

Co-Authored-By: Claude Sonnet 4.6 <noreply@anthropic.com>

- Docs: mark MUN-16 and WorkManager fixes as done; surface next task

- ROADMAP.md: add status column; mark items 1 and 1a complete;
  flag item 2 (dark mode) as next; add WorkManager as item 1a
- DEVELOPMENT.md: update active bug table with fix status for
  MUN-16 and WorkManager; keep dark mode and allowMainThreadQueries
  as open items

Co-Authored-By: Claude Sonnet 4.6 <noreply@anthropic.com>

- Docs: update roadmap with full dark mode fix scope

- Docs(roadmap): mark image-tap selection UX as done (item 2a)

Co-Authored-By: Claude Sonnet 4.6 <noreply@anthropic.com>

- Docs: add Compose migration guide and update architecture docs

- Add docs/COMPOSE_MIGRATION.md: full strangler-fig migration plan with
  8 prerequisites, 5 phases, screen-by-screen conversion map, adapter
  replacements, stepper strategy, testing strategy, and library removals.
  Build configuration section uses libs.versions.toml catalog aliases
  throughout (no raw Maven coordinates).
- docs/ARCHITECTURE.md: add target architecture diagram (§11), update
  tech debt table with Status column.
- docs/DEVELOPMENT.md: add §9 Compose conventions (file layout, state
  rules, theming, preview requirements, no-go list); add migration
  warning to "adding a new screen" guidance.
- docs/ROADMAP.md: mark items 2b/2c/2d done; expand medium-term to
  Compose prerequisites with dependency rationale; add 5 Compose phases
  (#13–17) and items 18–21; add 12-month milestone summary.
- README.md: add COMPOSE_MIGRATION.md to documentation index.
...
- Feat(theme): rebrand to AKILIMO/IITA identity with modern Material3 palette

colors.xml
- Primary: AKILIMO green #3D7600 (deepened from #61A229 for WCAG AA on white)
- Secondary: IITA lime-earth #586200 with #DCE87A container
- Tertiary: AKILIMO logo red #B5162A replacing the old blue accent
- Surfaces: tonal warm-green tint (#F5FCE9) for fresh agricultural feel
- Dedicated dark-primary (#9DD768), dark-secondary (#BFCA6A), dark-tertiary
  (#FFB3B4) tokens — no more reusing light container colors in dark theme
- Updated interactive state colors (ripple, hover, focus, pressed) to use
  primary green family instead of neutral black overlays
- Updated all dark surface colors with warm dark green undertone (#10140C)

themes.xml (light)
- Map all M3 color roles to new AKILIMO/IITA palette (primary, secondary,
  tertiary + their on/container counterparts)
- Add polished widget styles: pill-shaped buttons (50% corner), 12dp rounded
  cards, 8dp outlined TextInput, filter chips
- Add ShapeAppearance tokens: Button, Card, Large (24dp for bottom sheets)
- Fix toolbar font references to @font/google_sans_medium

themes.xml (night)
- Rebuild dark theme using dedicated dark color tokens; no shared references
  with light palette
- Proper M3 tertiary/container roles for dark mode

Layout hardcoded color cleanup (7 files)
- Replace @color/black icon tints with ?attr/colorPrimary
- Replace @color/color_primary with ?attr/colorPrimary
- Replace @color/color_primary_variant tints with ?attr/colorPrimary
- Replace @color/color_primary_variant text with ?attr/colorOnSurface

Co-Authored-By: Claude Sonnet 4.6 <noreply@anthropic.com>

- Feat(ui): network banner visible in all activities

BaseActivity - injectNetworkBannerIfNeeded()
- After onBindingReady() returns, programmatically add NetworkNotificationView
  to android.R.id.content (always a FrameLayout) with Gravity.TOP so it
  overlays every activity without touching individual layout files
- Apply WindowInsetsCompat.Type.statusBars() top inset via
  ViewCompat.setOnApplyWindowInsetsListener so the banner is fully visible
  below the status bar in edge-to-edge mode
- Skip injection if networkNotificationView is already set (HomeStepperActivity
  wires it from its XML layout — no double-banner)

NetworkNotificationView
- Replace hardcoded android.R.color.holo_red/green_light with themed
  color_error / color_primary and matching on-color text colors
- Use string resources lbl_no_internet / lbl_network_connected instead of
  hardcoded string literals
- Extract hide delay into a named Runnable so postDelayed can be reliably
  cancelled on showNoConnection() and onDetachedFromWindow()
- Set elevation from dimen_4 resource on init

strings.xml
- Expand lbl_no_internet to "No internet connection" (was "No internet!")
- Add lbl_network_connected = "Connected"
...
- Docs: Update release notes generator to use Ollama API
- Update changelog parsing and generation logic to work with new Ollama API
- Remove legacy LLM API usage
- Improve performance and conversion impact for Play Store release notes
- Enhance language support for Swahili (Kiswahili) and English (UK)

BREAKING CHANGE: The release notes generator now uses the Ollama API, which may require changes to your workflow or configuration. Please review the updated documentation and adjust your scripts accordingly.

- Docs:enhance release notes experience!:
- Improved changelog parsing performance and conversion impact
- Enhanced language support for Swahili (Kiswahili) and English (UK)
- Added new advertising ID permission for seamless updates

BREAKING CHANGE:
- Changes in the Ollama API usage may affect existing workflows or integrations.

- Docs: add technical codebase evaluation and architecture guide

- Docs: add technical evaluation and architecture documentation 

- Docs: add usage guide for scripts README

- Docs: add usage guide for scripts README 

- Docs: overhaul README and add developer architecture guides
...
Remove redundant `if` conditions for branch checks (`main`, `hotfix`) in the deployment pipeline, as the workflow is already triggered only for these branches.
...
- 🔧 chore(scripts): exclude ollama from release notes

Update the release note generation script to filter out ollama-related changes from user-facing descriptions. This ensures internal tooling details remain hidden from end users.

- ✨ feat(database): add user preferences entity

Add a new table to store user-specific preferences and settings, and add a corresponding DAO to access this data. The database version is bumped to 2 with a destructive migration strategy.
