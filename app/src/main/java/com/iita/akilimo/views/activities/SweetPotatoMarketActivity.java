...
public class SweetPotatoMarketActivity extends BaseActivity {
    ...
    protected void initComponent() {
        ...
        rdgUnitOfSalePotato.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(RadioGroup group, int radioIndex) {
                switch (radioIndex) {
                    case R.id.rd_per_kg:
...
//                nearestRounding = 100;
    ...
    private void showPotatoUnitPriceDialog(String currency, String uos) {
        ...
        dialog.findViewById(R.id.bt_cancel).setOnClickListener(v -> {
            dialog.dismiss();
            dialogOpen = false;
            if (potatoUnitOfSaleRadioIndex <= 0) {
